...
public interface TblSysDicMapper {
     ...
     * 查询单个对象
     */
    public TblSysDic select(TblSysDic tblSysDic);

    /**
     * 查询一个对象列表
     */
    public List<TblSysDic> selectList(TblSysDic tblSysDic);

    /**
     * 新增对象，返回增加的数量
     */
    public int insert(TblSysDic tblSysDic);

    /**
     * 通过主键修改对象，返回修改的数量，主键不可修改
     */
    public int updateByPrimaryKey(TblSysDic tblSysDic);

    /**
     * 删除对象，返回删除的数量
     */
    public int delete(TblSysDic tblSysDic);
     ...
     */
    public int insertBatch(List<HashMap> tblSysDic);

    /**
     * 批量更新,如果是mysql数据库，需要在jdbcUrl中设置allowMultiQueries=true参数才可以使用
     * 返回值为第一条更新语句的执行结果，并非所有批量更新的语句总和
     */
    public int updateBatch(List<HashMap> tblSysDic);

    /**
     * 根据主键查询单个对象
     */
    public TblSysDic selectByPrimaryKey(Integer id);
