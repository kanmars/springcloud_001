...
public class Shard<T>  { // S类封装了机器节点的信息 ，如name、password、ip、port等
    ...
    public static final HLogger logger= LoggerManager.getLoger("Shard");
    private TreeMap<Long, Node> nodes; // 虚拟节点到真实节点的映射
    private TreeMap<Long,Node> treeKey; //key到真实节点的映射
    private List<Node> shards = new ArrayList<Node>(); // 真实机器节点
    ...
    public void setShards(List<Node> shards) {
        this.shards = shards;
    }
...
//        printKeyTree();*/
    ...
    public  void printKeyTree(){
        for(Iterator<Long> it = treeKey.keySet().iterator();it.hasNext();){
            Long lo = it.next();
            logger.info("hash(" + lo + ")连接到主机->" + treeKey.get(lo));
        }
    ...
    public void init() { // 初始化一致性hash环
        ...
        for (int i = 0; i != shards.size(); ++i) { // 每个真实机器节点都需要关联虚拟节点
            ...
            logger.info("shard" + i + ":=" + shardInfo.name);
        }
    }
    //增加一个主机
    private void addS(Node s) {
        logger.info("增加主机" + s + "的变化：");
        for (int n = 0; n < NODE_NUM; n++)
    ...
    public void addS(Long lg,Node s){
        ...
        for(Iterator<Long> it=between.keySet().iterator();it.hasNext();){
            ...
            if(flag){
                ...
                logger.info("hash(" + lo + ")改变到->" + tail.get(tail.firstKey()));
            }else{
                treeKey.put(lo, nodes.get(lg));
                logger.info("hash(" + lo + ")改变到->" + tail.get(tail.firstKey()));
            }
    ...
    public void keyToNode(String key){
        ...
        logger.info(key + "（hash：" + hash(key) + "）连接到主机->" + tail.get(tail.firstKey()));
    }
    //映射key到真实节点
    public Integer getKey(String key){
        ...
        return Integer.parseInt( tail.get(tail.firstKey()).ip);
    ...
    private static Long hash(String key) {
        ...
        ByteBuffer buf = null;
        try {
            buf = ByteBuffer.wrap(key.getBytes(defaulCharset));
