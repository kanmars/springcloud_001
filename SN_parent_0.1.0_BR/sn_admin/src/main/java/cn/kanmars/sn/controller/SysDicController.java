...
import cn.kanmars.sn.logic.SysDicLogic;
...
public class SysDicController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("SysDicController");

    @Autowired
    private SysDicLogic sysDicLogic;

    @RequestMapping("/sysDicView.dhtml")
    public String init() {
        return "sysdic/sysdic";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        tblSysDic.setId(Integer.parseInt(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblSysDic = sysDicLogic.querySysDic(tblSysDic);
            logger.info("查询结果:tblSysDic:" + (tblSysDic != null ? JSONObject.fromObject(tblSysDic).toString() : "null"));
            Map result = new HashMap();
            Map tblSysDic_map = afterLogic(request, JSONObject.fromObject(tblSysDic));
            result.put("tblSysDic", tblSysDic_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblSysDic tblSysDic = new TblSysDic();
            try {
                MapObjTransUtils.mapToObj(tblSysDic, paramMap);
            } catch (Exception e) {
                logger.error("对象转化异常", e);
            }
            tblSysDic.setCreateTime(DateUtils.getCurrDateTime());
            tblSysDic.setUpTime(DateUtils.getCurrDateTime());
            try {
                String pk0 = SysSequenceUtils.generateStringId("tbl_sys_dic");
                tblSysDic.setId(Integer.parseInt(pk0));// 联合主键用逗号分割
            } catch (Exception e) {
                logger.error("对象转化异常", e);
            }
            logger.info("传入参数:tblSysDic:" + JSONObject.fromObject(tblSysDic).toString());
            int count = sysDicLogic.insertSysDic(tblSysDic);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblSysDic:" + JSONObject.fromObject(tblSysDic).toString());
            int count = sysDicLogic.updateSysDic(tblSysDic);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblSysDic tblSysDic = new TblSysDic();
                tblSysDic.setId(Integer.parseInt(s));// 联合主键用逗号分割
                try {
                    count += sysDicLogic.deleteSysDic(tblSysDic);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = sysDicLogic.queryPageSysDic(paramMap);
