...
import cn.kanmars.sn.logic.OperationLogLogic;
...
public class OperationLogController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("OperationLogController");

    @Autowired
    private OperationLogLogic operationLogLogic;

    @RequestMapping("/operationLogView.dhtml")
    public String init() {
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        tblOperationLog.setOperationId(Integer.parseInt(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblOperationLog = operationLogLogic.queryOperationLog(tblOperationLog);
            logger.info("查询结果:tblOperationLog:" + (tblOperationLog != null ? JSONObject.fromObject(tblOperationLog).toString() : "null"));
            Map result = new HashMap();
            Map tblOperationLog_map = afterLogic(request, JSONObject.fromObject(tblOperationLog));
            result.put("tblOperationLog", tblOperationLog_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblOperationLog tblOperationLog = new TblOperationLog();
            try {
                MapObjTransUtils.mapToObj(tblOperationLog, paramMap);
            } catch (Exception e) {
                logger.error("对象转化异常", e);
            }
            tblOperationLog.setOperationTime(DateUtils.getCurrDateTime());
            tblOperationLog.setCreateTime(DateUtils.getCurrDateTime());
            tblOperationLog.setUpdateTime(DateUtils.getCurrDateTime());
            try {
                String pk0 = SysSequenceUtils.generateStringId("tbl_operation_log");
                tblOperationLog.setOperationId(Integer.parseInt(pk0));// 联合主键用逗号分割
            } catch (Exception e) {
                logger.error("对象转化异常", e);
            }
            logger.info("传入参数:tblOperationLog:" + JSONObject.fromObject(tblOperationLog).toString());
            int count = operationLogLogic.insertOperationLog(tblOperationLog);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblOperationLog:" + JSONObject.fromObject(tblOperationLog).toString());
            int count = operationLogLogic.updateOperationLog(tblOperationLog);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblOperationLog tblOperationLog = new TblOperationLog();
                tblOperationLog.setOperationId(Integer.parseInt(s));// 联合主键用逗号分割
                try {
                    count += operationLogLogic.deleteOperationLog(tblOperationLog);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = operationLogLogic.queryPageOperationLog(paramMap);
    ...
    protected HashMap<String, Object> beferLogic(HttpServletRequest request, Map<String, Object> obj) throws Exception {
        ...
        for (Map.Entry<String, Object> e : obj.entrySet()) {
            ...
            if (key.equals("operationTime") && StringUtils.isNotEmpty(value)) {
                try {
                    ...
                    result.put("operationTime", dbDateStr);
                } catch(Exception ex) {
                    logger.error("格式转化失败["+key+"]["+value+"]", ex);
                    throw new Exception("格式转化失败["+key+"]["+value+"]");
                }
            }
            if (key.equals("operationTime_start") && StringUtils.isNotEmpty(value)) {
                try {
                    java.text.SimpleDateFormat sdf_page = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                    java.text.SimpleDateFormat sdf_db = new java.text.SimpleDateFormat("yyyyMMddHHmmss");
                    String dbDateStr=sdf_db.format(sdf_page.parse(value));
                    result.put("operationTime_start", dbDateStr);
                } catch(Exception ex) {
                    logger.error("格式转化失败["+key+"]["+value+"]", ex);
                    throw new Exception("格式转化失败["+key+"]["+value+"]");
                }
            }
            if (key.equals("operationTime_end") && StringUtils.isNotEmpty(value)) {
                try {
                    java.text.SimpleDateFormat sdf_page = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                    java.text.SimpleDateFormat sdf_db = new java.text.SimpleDateFormat("yyyyMMddHHmmss");
                    String dbDateStr=sdf_db.format(sdf_page.parse(value));
                    result.put("operationTime_end", dbDateStr);
                } catch(Exception ex) {
                    logger.error("格式转化失败["+key+"]["+value+"]", ex);
                    throw new Exception("格式转化失败["+key+"]["+value+"]");
                }
            ...
            if (key.equals("updateTime_start") && StringUtils.isNotEmpty(value)) {
                try {
                    java.text.SimpleDateFormat sdf_page = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                    java.text.SimpleDateFormat sdf_db = new java.text.SimpleDateFormat("yyyyMMddHHmmss");
                    String dbDateStr=sdf_db.format(sdf_page.parse(value));
                    result.put("updateTime_start", dbDateStr);
                } catch(Exception ex) {
                    logger.error("格式转化失败["+key+"]["+value+"]", ex);
                    throw new Exception("格式转化失败["+key+"]["+value+"]");
                }
            }
            if (key.equals("updateTime_end") && StringUtils.isNotEmpty(value)) {
                try {
                    java.text.SimpleDateFormat sdf_page = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                    java.text.SimpleDateFormat sdf_db = new java.text.SimpleDateFormat("yyyyMMddHHmmss");
                    String dbDateStr=sdf_db.format(sdf_page.parse(value));
                    result.put("updateTime_end", dbDateStr);
                } catch(Exception ex) {
                    logger.error("格式转化失败["+key+"]["+value+"]", ex);
                    throw new Exception("格式转化失败["+key+"]["+value+"]");
                }
    ...
    private Map<String, Object> afterLogicpreCell(HttpServletRequest request, Map<String, Object> result) throws Exception {
            if (result.keySet().contains("operationTime")) {
