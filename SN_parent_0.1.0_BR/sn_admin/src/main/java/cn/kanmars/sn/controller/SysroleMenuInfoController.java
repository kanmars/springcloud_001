...
import cn.kanmars.sn.logic.SysroleMenuInfoLogic;
 ...
 */
@Controller
@RequestMapping("/sysroleMenuInfo")
public class SysroleMenuInfoController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("SysroleMenuInfoController");

    @Autowired
    private SysroleMenuInfoLogic sysroleMenuInfoLogic;
    @Autowired
    private SysroleInfoLogic sysroleInfoLogic;
    @Autowired
    private SysmenuInfoLogic sysmenuInfoLogic;

    @RequestMapping("/sysroleMenuInfoView.dhtml")
    public String init() {
        return "sysrolemenuinfo/sysrolemenuinfo";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        TblSysroleMenuInfo tblSysroleMenuInfo = new TblSysroleMenuInfo();
        tblSysroleMenuInfo.setRoleNo(jsonObject.get("id_key").toString().split(",")[0]);// 联合主键用逗号分割
        tblSysroleMenuInfo.setMenuNo(jsonObject.get("id_key").toString().split(",")[1]);// 联合主键用逗号分割

        try {
            tblSysroleMenuInfo = sysroleMenuInfoLogic.querySysroleMenuInfo(tblSysroleMenuInfo);
            logger.info("查询结果:tblSysroleMenuInfo:" + (tblSysroleMenuInfo != null ? JSONObject.fromObject(tblSysroleMenuInfo).toString() : "null"));
            Map result = new HashMap();
            Map tblSysroleMenuInfo_map = afterLogic(request, JSONObject.fromObject(tblSysroleMenuInfo));
            result.put("tblSysroleMenuInfo", tblSysroleMenuInfo_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            tblSysmenuInfo.setMenuNo(jsonObject.get("menuNo").toString());// 联合主键用逗号分割
            try {
                tblSysmenuInfo = sysmenuInfoLogic.querySysmenuInfo(tblSysmenuInfo);
                if(tblSysmenuInfo==null){
                    mesg="请先添加菜单信息！";
                }
            ...
            TblSysroleMenuInfo tblSysroleMenuInfo = new TblSysroleMenuInfo();
            try {
                MapObjTransUtils.mapToObj(tblSysroleMenuInfo, paramMap);
            } catch (Exception e) {
                logger.error("对象转化异常", e);
            }
            tblSysroleMenuInfo.setCreateTime(DateUtils.getCurrDateTime());
            tblSysroleMenuInfo.setUpTime(DateUtils.getCurrDateTime());
            logger.info("传入参数:tblSysroleMenuInfo:" + JSONObject.fromObject(tblSysroleMenuInfo).toString());
            int count = sysroleMenuInfoLogic.insertSysroleMenuInfo(tblSysroleMenuInfo);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblSysroleMenuInfo:" + JSONObject.fromObject(tblSysroleMenuInfo).toString());
            int count = sysroleMenuInfoLogic.updateSysroleMenuInfo(tblSysroleMenuInfo);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblSysroleMenuInfo tblSysroleMenuInfo = new TblSysroleMenuInfo();
                tblSysroleMenuInfo.setRoleNo(s.split(",")[0]);// 联合主键用逗号分割
                tblSysroleMenuInfo.setMenuNo(s.split(",")[1]);// 联合主键用逗号分割
                try {
                    count += sysroleMenuInfoLogic.deleteSysroleMenuInfo(tblSysroleMenuInfo);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = sysroleMenuInfoLogic.queryPageSysroleMenuInfo(paramMap);
