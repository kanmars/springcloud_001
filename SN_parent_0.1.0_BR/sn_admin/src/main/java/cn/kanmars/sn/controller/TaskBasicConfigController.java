...
import cn.kanmars.sn.logic.TaskBasicConfigLogic;
...
public class TaskBasicConfigController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("TaskBasicConfigController");

    @Autowired
    private TaskBasicConfigLogic taskBasicConfigLogic;

    @RequestMapping("/taskBasicConfigView.dhtml")
    public String init() {
        return "taskbasicconfig/taskbasicconfig";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        TblTaskBasicConfig tblTaskBasicConfig = new TblTaskBasicConfig();
        tblTaskBasicConfig.setId(jsonObject.get("id_key").toString());// 联合主键用逗号分割

        try {
            tblTaskBasicConfig = taskBasicConfigLogic.queryTaskBasicConfig(tblTaskBasicConfig);
            logger.info("查询结果:tblTaskBasicConfig:" + (tblTaskBasicConfig != null ? JSONObject.fromObject(tblTaskBasicConfig).toString() : "null"));
            Map result = new HashMap();
            Map tblTaskBasicConfig_map = afterLogic(request, JSONObject.fromObject(tblTaskBasicConfig));
            result.put("tblTaskBasicConfig", tblTaskBasicConfig_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblTaskBasicConfig tblTaskBasicConfig = new TblTaskBasicConfig();
            try {
                MapObjTransUtils.mapToObj(tblTaskBasicConfig, paramMap);
            } catch (Exception e) {
                logger.error("对象转化异常", e);
            }
            tblTaskBasicConfig.setConfigTime(DateUtils.getCurrDateTime());
            tblTaskBasicConfig.setUpTime(DateUtils.getCurrDateTime());
            try {
                String pk0 = SysSequenceUtils.generateStringId("tbl_task_basic_config");
                tblTaskBasicConfig.setId(pk0);// 联合主键用逗号分割
            } catch (Exception e) {
                logger.error("对象转化异常", e);
            }
            logger.info("传入参数:tblTaskBasicConfig:" + JSONObject.fromObject(tblTaskBasicConfig).toString());
            int count = taskBasicConfigLogic.insertTaskBasicConfig(tblTaskBasicConfig);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblTaskBasicConfig:" + JSONObject.fromObject(tblTaskBasicConfig).toString());
            int count = taskBasicConfigLogic.updateTaskBasicConfig(tblTaskBasicConfig);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblTaskBasicConfig tblTaskBasicConfig = new TblTaskBasicConfig();
                tblTaskBasicConfig.setId(s);// 联合主键用逗号分割
                try {
                    count += taskBasicConfigLogic.deleteTaskBasicConfig(tblTaskBasicConfig);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = taskBasicConfigLogic.queryPageTaskBasicConfig(paramMap);
    ...
    protected HashMap<String, Object> beferLogic(HttpServletRequest request, Map<String, Object> obj) throws Exception {
        ...
        for (Map.Entry<String, Object> e : obj.entrySet()) {
            ...
            if (key.equals("configTime") && StringUtils.isNotEmpty(value)) {
                try {
                    ...
                    result.put("configTime", dbDateStr);
                } catch(Exception ex) {
    ...
    private Map<String, Object> afterLogicpreCell(HttpServletRequest request, Map<String, Object> result) throws Exception {
            if (result.keySet().contains("runStatus")) {
                String key = "runStatus";
                Object value = result.get(key);
                if(value!=null&&StringUtils.isNotEmpty(value.toString())){
                    try {
                        String runStatus_name = null;
                        JSONObject json = JSONObject.fromObject("{'2':'同任务执行中抛弃新来任务','1':'同任务串行执行','0':'无限制执行'}");
                        runStatus_name = json.get(value.toString())==null?value.toString():(String)json.get(value.toString());
                        result.put("runStatus_name", runStatus_name);
                    } catch(Exception ex) {
                        logger.error("格式转化失败["+key+"]["+value+"]", ex);
                        throw new Exception("格式转化失败["+key+"]["+value+"]");
                    }
            ...
            if (result.keySet().contains("status")) {
                ...
                if(value!=null&&StringUtils.isNotEmpty(value.toString())){
                    try {
                        ...
                        JSONObject json = JSONObject.fromObject("{'2':'禁止','1':'开启'}");
                        status_name = json.get(value.toString())==null?value.toString():(String)json.get(value.toString());
            ...
            if (result.keySet().contains("configTime")) {
                String key = "configTime";
                Object value = result.get(key);
