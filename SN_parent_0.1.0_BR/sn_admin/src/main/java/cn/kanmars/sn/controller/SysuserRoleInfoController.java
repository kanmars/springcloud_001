...
import cn.kanmars.sn.logic.SysroleInfoLogic;
import cn.kanmars.sn.logic.SysuserRoleInfoLogic;
 ...
 */
@Controller
@RequestMapping("/sysuserRoleInfo")
public class SysuserRoleInfoController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("SysuserRoleInfoController");

    @Autowired
    private SysuserRoleInfoLogic sysuserRoleInfoLogic;
    @Autowired
    private UserInfoLogic userInfoLogic;
    @Autowired
    private SysroleInfoLogic sysroleInfoLogic;

    @RequestMapping("/sysuserRoleInfoView.dhtml")
    public String init() {
        return "sysuserroleinfo/sysuserroleinfo";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        TblSysuserRoleInfo tblSysuserRoleInfo = new TblSysuserRoleInfo();
        tblSysuserRoleInfo.setUserNo(jsonObject.get("id_key").toString().split(",")[0]);// 联合主键用逗号分割
        tblSysuserRoleInfo.setRoleNo(jsonObject.get("id_key").toString().split(",")[1]);// 联合主键用逗号分割

        try {
            tblSysuserRoleInfo = sysuserRoleInfoLogic.querySysuserRoleInfo(tblSysuserRoleInfo);
            logger.info("查询结果:tblSysuserRoleInfo:" + (tblSysuserRoleInfo != null ? JSONObject.fromObject(tblSysuserRoleInfo).toString() : "null"));
            Map result = new HashMap();
            Map tblSysuserRoleInfo_map = afterLogic(request, JSONObject.fromObject(tblSysuserRoleInfo));
            result.put("tblSysuserRoleInfo", tblSysuserRoleInfo_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            tblSysuserInfo.setUserNo(jsonObject.get("userNo").toString());

            try {
                tblSysuserInfo = userInfoLogic.queryUserInfo(tblSysuserInfo);
                if(tblSysuserInfo==null){
                    mesg =  "请先添加用户信息！";
                }
                logger.info("查询结果:tblSysroleInfo:" + (tblSysuserInfo != null ? JSONObject.fromObject(tblSysuserInfo).toString() : "null"));
            } catch (Exception e1) {
                logger.error("处理失败", e1);
                ajaxJsonMessage(request, response, ERROR,"操作异常,请联系管理员！", QUERY);
                return;
            }
            if(!mesg.equals("")){
                ajaxJsonMessage(request, response, ERROR, mesg, SAVE);
                return;
            }
            TblSysroleInfo tblSysroleInfo = new TblSysroleInfo();
            tblSysroleInfo.setRoleNo(jsonObject.get("roleNo").toString());

            try {
                tblSysroleInfo = sysroleInfoLogic.querySysroleInfo(tblSysroleInfo);
                if(tblSysroleInfo==null){
                    mesg="请先添加角色信息！";
                }
                logger.info("查询结果:tblSysroleInfo:" + (tblSysroleInfo != null ? JSONObject.fromObject(tblSysroleInfo).toString() : "null"));
            } catch (Exception e1) {
                logger.error("处理失败", e1);
                ajaxJsonMessage(request, response, ERROR,"操作异常,请联系管理员！", QUERY);
                return;
            ...
            TblSysuserRoleInfo tblSysuserRoleInfo = new TblSysuserRoleInfo();
            try {
                MapObjTransUtils.mapToObj(tblSysuserRoleInfo, paramMap);
            } catch (Exception e) {
                logger.error("对象转化异常", e);
            }
            tblSysuserRoleInfo.setCreateTime(DateUtils.getCurrDateTime());
            tblSysuserRoleInfo.setUpTime(DateUtils.getCurrDateTime());
            logger.info("传入参数:tblSysuserRoleInfo:" + JSONObject.fromObject(tblSysuserRoleInfo).toString());
            int count = sysuserRoleInfoLogic.insertSysuserRoleInfo(tblSysuserRoleInfo);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblSysuserRoleInfo:" + JSONObject.fromObject(tblSysuserRoleInfo).toString());
            int count = sysuserRoleInfoLogic.updateSysuserRoleInfo(tblSysuserRoleInfo);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblSysuserRoleInfo tblSysuserRoleInfo = new TblSysuserRoleInfo();
                tblSysuserRoleInfo.setUserNo(s.split(",")[0]);// 联合主键用逗号分割
                tblSysuserRoleInfo.setRoleNo(s.split(",")[1]);// 联合主键用逗号分割
                try {
                    count += sysuserRoleInfoLogic.deleteSysuserRoleInfo(tblSysuserRoleInfo);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = sysuserRoleInfoLogic.queryPageSysuserRoleInfo(paramMap);
