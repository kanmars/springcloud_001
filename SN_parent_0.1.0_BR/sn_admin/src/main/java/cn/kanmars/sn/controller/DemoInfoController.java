...
import cn.kanmars.sn.logic.DemoInfoLogic;
...
public class DemoInfoController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("DemoInfoController");

    @Autowired
    private DemoInfoLogic demoInfoLogic;

    @RequestMapping("/demoInfoView.dhtml")
    public String init() {
        return "demoinfo/demoinfo";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        TblDemoInfo tblDemoInfo = new TblDemoInfo();
        tblDemoInfo.setDemoId(Integer.parseInt(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblDemoInfo = demoInfoLogic.queryDemoInfo(tblDemoInfo);
            logger.info("查询结果:tblDemoInfo:" + (tblDemoInfo != null ? JSONObject.fromObject(tblDemoInfo).toString() : "null"));
            Map result = new HashMap();
            Map tblDemoInfo_map = afterLogic(request, JSONObject.fromObject(tblDemoInfo));
            result.put("tblDemoInfo", tblDemoInfo_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblDemoInfo tblDemoInfo = new TblDemoInfo();
            try {
                MapObjTransUtils.mapToObj(tblDemoInfo, paramMap);
            } catch (Exception e) {
                logger.error("对象转化异常", e);
            }
            tblDemoInfo.setCreateTime(DateUtils.getCurrDateTime());
            try {
                String pk0 = SysSequenceUtils.generateStringId("tbl_demo_info");
                tblDemoInfo.setDemoId(Integer.parseInt(pk0));// 联合主键用逗号分割
            } catch (Exception e) {
                logger.error("对象转化异常", e);
            }
            logger.info("传入参数:tblDemoInfo:" + JSONObject.fromObject(tblDemoInfo).toString());
            int count = demoInfoLogic.insertDemoInfo(tblDemoInfo);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblDemoInfo:" + JSONObject.fromObject(tblDemoInfo).toString());
            int count = demoInfoLogic.updateDemoInfo(tblDemoInfo);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblDemoInfo tblDemoInfo = new TblDemoInfo();
                tblDemoInfo.setDemoId(Integer.parseInt(s));// 联合主键用逗号分割
                try {
                    count += demoInfoLogic.deleteDemoInfo(tblDemoInfo);
                } catch(Exception e) {
    ...
    protected HashMap<String, Object> beferLogic(HttpServletRequest request, Map<String, Object> obj) throws Exception {
        ...
        for (Map.Entry<String, Object> e : obj.entrySet()) {
            ...
            if (key.equals("demoMoney") && StringUtils.isNotEmpty(value)) {
                try {
                    result.put("demoMoney", new BigDecimal(MoneyFormatUtils.format(value, "###0.##", "y2f")));
                } catch(Exception ex) {
    ...
    private Map<String, Object> afterLogicpreCell(HttpServletRequest request, Map<String, Object> result) throws Exception {
            if (result.keySet().contains("demoMoney")) {
                String key = "demoMoney";
