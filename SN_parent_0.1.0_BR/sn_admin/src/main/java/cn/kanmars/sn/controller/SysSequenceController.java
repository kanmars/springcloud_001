...
import cn.kanmars.sn.logic.SysSequenceLogic;
...
public class SysSequenceController extends AdvancedAjaxBaseController  {

    protected HLogger logger = LoggerManager.getLoger("SysSequenceController");

    @Autowired
    private SysSequenceLogic sysSequenceLogic;

    @RequestMapping("/sysSequenceView.dhtml")
    public String init() {
        return "syssequence/syssequence";
    }
    ...
    public void queryObject(HttpServletRequest request, HttpServletResponse response) {
        ...
        tblSysSequence.setId(Integer.parseInt(jsonObject.get("id_key").toString()));// 联合主键用逗号分割

        try {
            tblSysSequence = sysSequenceLogic.querySysSequence(tblSysSequence);
            logger.info("查询结果:tblSysSequence:" + (tblSysSequence != null ? JSONObject.fromObject(tblSysSequence).toString() : "null"));
            Map result = new HashMap();
            Map tblSysSequence_map = afterLogic(request, JSONObject.fromObject(tblSysSequence));
            result.put("tblSysSequence", tblSysSequence_map);
    ...
    public void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            TblSysSequence tblSysSequence = new TblSysSequence();
            try {
                MapObjTransUtils.mapToObj(tblSysSequence, paramMap);
            } catch (Exception e) {
                logger.error("对象转化异常", e);
            }
            tblSysSequence.setCreateTime(DateUtils.getCurrDateTime());
            tblSysSequence.setUpTime(DateUtils.getCurrDateTime());
            try {
                String pk0 = SysSequenceUtils.generateStringId("tbl_sys_sequence");
                tblSysSequence.setId(Integer.parseInt(pk0));// 联合主键用逗号分割
            } catch (Exception e) {
                logger.error("对象转化异常", e);
            }
            logger.info("传入参数:tblSysSequence:" + JSONObject.fromObject(tblSysSequence).toString());
            int count = sysSequenceLogic.insertSysSequence(tblSysSequence);
    ...
    public void edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logger.info("传入参数:tblSysSequence:" + JSONObject.fromObject(tblSysSequence).toString());
            int count = sysSequenceLogic.updateSysSequence(tblSysSequence);
            if (count == 0) {
    ...
    public void del(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            for (String s : id_key.split(";")) { // 多个主键用分号分割
                logger.info("尝试删除:" + s);
                TblSysSequence tblSysSequence = new TblSysSequence();
                tblSysSequence.setId(Integer.parseInt(s));// 联合主键用逗号分割
                try {
                    count += sysSequenceLogic.deleteSysSequence(tblSysSequence);
                } catch(Exception e) {
    ...
    public void search(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            paramMap = sysSequenceLogic.queryPageSysSequence(paramMap);
